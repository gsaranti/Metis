...
- **Codex plugin** at `.codex/`. Mirror of the Claude Code side: `SessionStart` / `UserPromptSubmit` / `Stop` hooks write to `iris-codex-chat.md` and `iris-codex-last.md`; four skills (`$iris-sync`, `$iris-relay`, `$iris-off`, `$iris-on`) parallel the Claude Code set. Hook commands use `${PLUGIN_ROOT}` for plugin-root resolution; like Claude Code, the `Stop` hook reads `last_assistant_message` from the payload directly. Skill invocation policy moves to `agents/openai.yaml` as `policy.allow_implicit_invocation: false` (Codex's equivalent of `disable-model-invocation: true`). Codex skills can only reach files inside their own directory, so each toggle skill ships its own copy of `iris-toggle.sh` under `references/`.
...
- `jq` is a runtime dependency on both sides — the hook scripts use it to parse hook payloads. `brew install jq` on macOS, `apt install jq` on Linux.
