...
- **Context.** Quote the *minimum relevant passage* — a task that pastes 500 words of source material has almost certainly buried the 50 that are load-bearing. When the relevant passage is long, summarize in your own words and keep one or two load-bearing quotes verbatim. For tasks that touch existing code, file:line refs from a `code-explorer` report belong here alongside source-doc excerpts — the cited line is the load-bearing passage, the surrounding text the minimum context the implementer needs.
