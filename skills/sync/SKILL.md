...
- `metis:propagating-spec-changes` — read before proposing edits.
- `metis:writing-decisions` — invoked per accepted substantive change and per accepted upstream `BUILD.md` edit.
...
3. **Record.** Per accepted substantive change (and per accepted cosmetic *batch*), write a `decisions/` entry via `metis:writing-decisions` naming the upstream change, the downstream artifacts edited, and the consequences.
