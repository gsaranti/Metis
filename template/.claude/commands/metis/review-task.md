...
The trailing prompt is forwarded to the subagent verbatim. The four discipline rules (augment / flag scope expansion / acknowledge use / resolve named skills) are enforced inside `task-reviewer.md` — this command does not re-check them, but does carry the subagent's prompt-usage acknowledgment through to the user in the return.
