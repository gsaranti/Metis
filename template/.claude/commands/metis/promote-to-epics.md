...
The command may carry a trailing free-text prompt, e.g. `/metis:promote-to-epics "keep auth and users as one epic; split billing into separate payment and subscription epics"`. Four rules:

1. **Augment, do not replace.** The existing task set and `BUILD.md` remain authoritative. If the prompt asks the grouping to leave tasks out of the epic layout or to create an epic that covers no tasks, flag the conflict rather than comply.
2. **Flag scope expansion.** If the prompt asks the command to edit task bodies during migration (rescope, rewrite acceptance criteria), note the expansion and decline. Migration is a structural move; content changes are `/metis:sync` territory.
3. **Acknowledge use explicitly.** State in the return how the prompt shaped the grouping — which seams it pinned, which merges it forced.
4. **Resolve named skills.** The prompt may name additional skills for this turn. Resolve each reference the same way any skill reference is resolved, across whatever skill sources the runtime exposes. Acknowledge each invoked skill's influence; flag unresolvable names rather than guessing. User-referenced skills augment the decomposing/writing skills; they do not override the exit-criterion test or the capability-not-category framing.
