...
The command may carry a trailing free-text prompt, e.g. `/metis:walk-open-items "prioritize the auth items; defer anything billing-related"`. Four rules:

1. **Augment, do not replace.** The items' captured framings remain authoritative. If the prompt asks the walk to take a side the docs do not support, flag the conflict rather than silently resolving.
2. **Flag scope expansion.** If the prompt asks the walk to edit surfaces beyond the cited docs (e.g., pre-writing `BUILD.md` sections), note the expansion and decline.
3. **Acknowledge use explicitly.** State in the return how the prompt shaped the walk — which items were prioritized, which were deferred per the prompt.
4. **Resolve named skills.** The prompt may name additional skills for this turn. Resolve each reference the same way any skill reference is resolved, across whatever skill sources the runtime exposes. Acknowledge each invoked skill's influence; flag unresolvable names rather than guessing. User-referenced skills augment `walking-open-items`; they do not override the three-register rule or the user-in-loop threshold.
