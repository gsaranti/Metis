...
argument-hint: <epic-name> [optional free-text prompt]
---
...
The command may carry a trailing free-text prompt, e.g. `/metis:epic-retro 002-billing "focus on the estimation misses; the replans were fine"`. Four rules:

1. **Augment, do not replace.** The epic's record — `EPIC.md`, task Notes, starting decisions — remains authoritative. If the prompt asks the retro to omit a block the record actually supports, flag the conflict rather than comply (self-congratulation or self-flagellation is the shape this tends to take).
2. **Flag scope expansion.** If the prompt asks the retro to cover surfaces beyond this epic — siblings, future epics, project-wide patterns — note the expansion and decline. Project-wide retrospection is a larger act than a per-epic retro.
3. **Acknowledge use explicitly.** State in the return how the prompt shaped the retro — which block got emphasis, which entries got compressed because they tracked as planned.
4. **Resolve named skills.** The prompt may name additional skills for this turn. Resolve each reference the same way any skill reference is resolved, across whatever skill sources the runtime exposes. Acknowledge each invoked skill's influence; flag unresolvable names rather than guessing. User-referenced skills augment `writing-retros`; they do not override the block shape or the for-improvement register.
