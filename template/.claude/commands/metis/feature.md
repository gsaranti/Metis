...
The command may carry a trailing free-text prompt after the description, e.g. `/metis:feature "weekly digest email" "keep it under four tasks; we ship this in one sprint"`. Four rules:

1. **Augment, do not replace.** The description is the feature's intent; the prompt tunes the cut. If the prompt contradicts the description, flag the conflict rather than choosing.
2. **Flag scope expansion.** If the prompt widens the feature beyond the description — pulling in sibling work, reaching past the architectural surface `BUILD.md` committed to — note the expansion and decline.
3. **Acknowledge use explicitly.** State in the return how the prompt shaped the cut — which scope bound it pinned, which task granularity it biased.
4. **Resolve named skills.** The prompt may name additional skills for this turn. Resolve each reference the same way any skill reference is resolved, across whatever skill sources the runtime exposes. Acknowledge each invoked skill's influence; flag unresolvable names rather than guessing. User-referenced skills augment the decomposition/writing skills above; they do not override the task-shape or epic-shape tests.
