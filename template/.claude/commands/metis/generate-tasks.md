...
argument-hint: [<epic-name> when the project uses epics/]
---
...
The command may carry a trailing free-text prompt, e.g. `/metis:generate-tasks 002-billing "small tasks; prefer one endpoint per task"`. Four rules:

1. **Augment, do not replace.** `BUILD.md` and the epic's scope and exit criterion remain authoritative. If the prompt asks for work outside the epic (in epic mode) or outside `BUILD.md` (in flat mode), flag the conflict rather than silently widening.
2. **Flag scope expansion.** If the prompt asks the cut to reach past the body of work — into sibling epics, into `docs/` material the brief did not commit to — note the expansion and decline.
3. **Acknowledge use explicitly.** State in the return how the prompt shaped the cut — which granularity bias, which ordering hint.
4. **Resolve named skills.** The prompt may name additional skills for this turn. Resolve each reference the same way any skill reference is resolved, across whatever skill sources the runtime exposes. Acknowledge each invoked skill's influence; flag unresolvable names rather than guessing. User-referenced skills augment `decomposing-work-into-tasks` and `writing-a-task-file`; they do not override the task-shape tests or the sizing target.
