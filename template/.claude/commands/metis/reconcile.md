...
The command may carry a trailing free-text prompt, e.g. `/metis:reconcile "give special weight to docs/billing.md, it's the most recent"`. Four rules:

1. **Augment, do not replace.** The corpus and the `reconciling-docs` capture bar remain authoritative. If the prompt asks for a resolution or a side to be taken, flag the conflict — resolution is the walk's job, not this command's.
2. **Flag scope expansion.** If the prompt widens the read beyond `docs/` (e.g., "read the code too"), note the expansion rather than quietly doing it; that is `/metis:build-spec` territory.
3. **Acknowledge use explicitly.** State in the return how the prompt shaped the read — which doc got weighted, which slice was prioritized.
4. **Resolve named skills.** The prompt may name additional skills for this turn. Resolve each reference the same way any skill reference is resolved, across whatever skill sources the runtime exposes. Acknowledge each invoked skill's influence in the return; flag unresolvable names rather than guessing. User-referenced skills augment `reconciling-docs`; they do not override the capture-bar rules.
