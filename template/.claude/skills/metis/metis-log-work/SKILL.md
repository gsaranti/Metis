...
- `logging-external-work` — read before reconciling.
- `writing-decisions` — invoked only when an architecture-level trigger fires. Not every log-work produces a decision.
...
- **Named task files.** Notes appends and `status` transitions per claim type (see `logging-external-work`).
- **Retroactive task files.** Per `logging-external-work` shape.
- **CRUD artifacts.** Split, merge, or add per `logging-external-work`.
