...
On `SIZE_CLASS=large`, apply the slicing guidance in `reconciling-docs`.

## Load

- The source docs under `docs/`.
- On `rereconcile`, the prior `docs/CONTRADICTIONS.md` and `docs/QUESTIONS.md`. Re-reconcile preserves prior items and re-checks them against the current corpus (per the `reconciling-docs` skill) rather than starting over.
...
Invoke `reconciling-docs` by reference — read the skill file before drafting any of the output artifacts.
