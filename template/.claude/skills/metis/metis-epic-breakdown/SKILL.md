...
- `decomposing-build-into-epics` — read before proposing the epic set.
- `writing-an-epic-file` — read before scaffolding files.
...
2. **Scaffold.** On approval, create `epics/NNN-kebab-name/` for each, with `EPIC.md` populated per `writing-an-epic-file`. Do not create `tasks/` subdirectories.
