---
name: logging-external-work
description: Reference for logging code written outside the Metis loop — reconciling the user's description against `git diff` into task-file updates, a retroactive task when no existing task matches, and a decision when the diff is architectural.
