---
name: propagating-spec-changes
description: Reference for propagating one upstream spec change through its downstream artifacts — detecting what the change reaches via `docs_refs` and frontmatter hashes, classifying each candidate as cosmetic or substantive, cascading by task status with `done` tasks absorbed as new work or a superseding decision rather than in-place edits, and recording each accepted change as a decision.
